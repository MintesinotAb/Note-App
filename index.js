let notes = [
{
    id: new Date(),
    title: 'Sample Note',
    body: 'This is a description for our note',
    bgColor: 'pink'
}
...
const cancelEdit = (notesDiv) => {
    ...
    const editButton = notesDiv.querySelector('button.edit');
    editButton.innerHTML = 'Edit Note';
    ...
    bodyP.innerHTML = note.body;
    editButton.onclick = () => editNote(noteDiv);
    deleteButton.onclick = () => deleteNote(noteDiv);S
...
const editNote = (noteDiv, editSave = false) => {
    ...
    deleteButton.onclick = () => cancleEdit(noteDiv);
    editButton.onclick = () => editNote(noteDiv, true);

    if (editSave){
           const note = notes.find(note =>note.id == noteDive.id);
